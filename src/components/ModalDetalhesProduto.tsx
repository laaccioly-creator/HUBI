...
export const ModalDetalhesProduto: React.FC<ModalDetalhesProdutoProps> = ({
  ...
  if (apenasGrade) {
    return (
      <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in">
        <div className="bg-white md:bg-slate-900 border border-slate-200 md:border-slate-800 rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-150 text-slate-800 md:text-slate-100">
          {/* Header */}
          <div className="p-4 sm:px-6 border-b border-slate-200 md:border-slate-800 flex items-center justify-between bg-white md:bg-slate-900/90">
            <div className="flex items-center gap-2.5">
              <div className="w-9 h-9 rounded-2xl bg-indigo-50 md:bg-indigo-500/15 text-indigo-600 md:text-indigo-400 flex items-center justify-center font-bold">
                <Layers className="w-5 h-5" />
              </div>
              <div>
                <h3 className="font-bold text-sm sm:text-base text-slate-800 md:text-slate-100 line-clamp-1">{produto.nome}</h3>
                <span className="text-[11px] text-slate-500 md:text-slate-400">Estoque por Variação / Grade</span>
              </div>
            </div>
            <button
              type="button"
          ...
          <div className="p-4 border-t border-slate-200 md:border-slate-800 bg-white md:bg-slate-900/90 flex items-center justify-between">
            <div className="text-xs text-slate-500 md:text-slate-400">
              Total em Grade:{' '}
              <strong className="text-emerald-600 md:text-emerald-400 font-black text-sm">{estoqueReal} {produto.tipo_unidade || 'un'}</strong>
            </div>
            <button
              type="button"
              onClick={onClose}
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in">
      <div className="bg-white md:bg-slate-900 border border-slate-200 md:border-slate-800 rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-150 text-slate-800 md:text-slate-100">
        {/* Header do Modal */}
        <div className="p-4 sm:px-6 border-b border-slate-200 md:border-slate-800 flex items-center justify-between bg-white md:bg-slate-900/90">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-sm sm:text-base text-slate-800 md:text-slate-100 line-clamp-1">{produto.nome}</h3>
              <span className="text-[11px] text-slate-500 md:text-slate-400">Consulta e Ficha Técnica do Produto</span>
            </div>
          </div>
          <button
            type="button"
        ...
        {/* Rodapé */}
        <div className="p-4 border-t border-slate-200 md:border-slate-800 bg-white md:bg-slate-900/90 flex justify-end">
          <button
            type="button"
            onClick={onClose}
            className="px-5 py-2 rounded-xl bg-slate-100 md:bg-slate-800 hover:bg-slate-200 md:hover:bg-slate-700 text-slate-700 md:text-slate-200 text-xs font-bold transition cursor-pointer"
