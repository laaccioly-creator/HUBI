...
export const ModalGerenciarCategorias: React.FC<ModalGerenciarCategoriasProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in">
      <div className="bg-white md:bg-slate-900 border border-slate-200 md:border-slate-800 rounded-3xl w-full max-w-md p-6 space-y-5 shadow-2xl text-slate-800 md:text-slate-100">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-200 md:border-slate-800 pb-3">
          <div className="flex items-center gap-2.5 text-emerald-600 md:text-emerald-400">
            <FolderPlus className="w-5 h-5" />
            <h3 className="font-bold text-base text-slate-800 md:text-slate-100">Gerenciar Categorias</h3>
          </div>
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-slate-700 md:hover:text-white p-1 rounded-lg hover:bg-slate-100 md:hover:bg-slate-800"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {erroMsg && (
          <div className="p-3 bg-rose-500/10 border border-rose-500/30 rounded-xl flex items-center gap-2 text-xs text-rose-600 md:text-rose-300">
        ...
        <form onSubmit={handleCriarCategoria} className="space-y-2">
          <label className="text-xs font-semibold text-slate-600 md:text-slate-300 block">Criar Nova Categoria:</label>
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 bg-slate-50 md:bg-slate-800 border border-slate-200 md:border-slate-700 rounded-xl px-3.5 py-2 text-xs text-slate-800 md:text-slate-100 placeholder:text-slate-400 md:placeholder:text-slate-500 focus:outline-none focus:border-emerald-500"
            />
            <button
              type="submit"
              disabled={salvando}
