...
export const PedidosListaMobile: React.FC<PedidosListaMobileProps> = ({
  ...
  return (
    <div className="flex flex-col h-full bg-white text-slate-900 overflow-hidden select-none">
        ...
        {[
          ...
          return (
            <button
              key={f.id}
              type="button"
              onClick={() => setStatusSelecionados([f.id])}
              className={`px-3 py-1.5 rounded-xl font-bold whitespace-nowrap transition flex items-center gap-1.5 shrink-0 cursor-pointer ${
                isActive
      ...
      {/* 3.1. Filtros Rápidos Complementares (Vendedores & Multi-status) */}
      <div className="px-4 py-1.5 border-b border-slate-100 bg-slate-50 flex items-center justify-between shrink-0 text-xs">
        <button
          type="button"
        ...
        >
          ...
          <span>{vendedoresSelecionados.includes('todos') ? 'Todos os Vendedores' : `${vendedoresSelecionados.length} vendedor(es)`}</span>
          <ChevronDown className="w-3.5 h-3.5 text-slate-400" />
        </button>
